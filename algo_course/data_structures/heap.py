...
class Heap:
    ...
    def max_heap_insert(self, key):
        """
        Insert the element with the highest key.
        Code by Victor Povazuk.
        """
        self._heap_size += 1
        self._heap[self._heap_size - 1] = -99999
        self.heap_increase_key(self._heap_size - 1, key)

    def heap_increase_key(self, i, key):
        """
        Increase the key for a given element.
        Code by Victor Povazuk.
        """
        self._heap[i] = key
        while i > 0 and self._heap[self.parent(i)] < self._heap[i]:
            self._heap[i], self._heap[self.parent(i)] = self._heap[self.parent(i)], \
                                                        self._heap[i]
            i = self.parent(i)

    def extract_max(self):
        """
       Extract the element with the max key.
        Code by Victor Povazuk.
        """
        maxi = self._heap[0]
        self._heap[0] = self._heap[self._heap_size - 1]
        # pop last element or not?
        self._heap_size -= 1
        self.max_heapify(0)
        return maxi
